...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    - in `progress` mode, groups and forum topics retain each completed assistant commentary/preamble as a normal message when automatic source replies are permitted (`messages.groupChat.visibleReplies: "automatic"`). This does not require verbose tool summaries, `replyToMode`, or an active preview. When a preview is available, the same commentary updates its headline; `streaming.progress.commentary: true` also includes it in the draft's activity history. DM commentary behavior is unchanged.
